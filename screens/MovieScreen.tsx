    ...
    return(
      <>
        ...
        {movieDetails !== undefined && <ScrollView className="bg-slate-950">
          ...
          <View>
            <Image  
              source={{uri: getImg(movieDetails?.poster_path, 500)}} 
              style={{width: width, height:height/2  }}  
            />
            <LinearGradient 
              colors={['transparent', 'rgba(0,0,20, 0.5)', 'rgba(0, 0, 20, .8)']} 
          ...
          <View className="flex items-center justify-center flex-1 -mt-24">
            {/* Title */}
            <Text className="p-3 text-3xl font-bold text-white">{movieDetails?.title}</Text>
            ...
            <View className="flex flex-row items-center justify-center flex-1 mb-2">
              ...
              <View className="flex-row items-center">
                <Text className="text-lg text-white">{movieDetails?.vote_average}</Text>
            ...
            <View className="flex flex-row items-center justify-center flex-1">
              {movieDetails.genres!==undefined &&  movieDetails?.genres?.map((genre, index) =>{
                  return(
                    <TouchableWithoutFeedback key={index}>
                      <Text className="px-3 py-1 m-1 text-white rounded-xl bg-black/40 ">{genre?.name}</Text>
            ...
            {/* Overview */}
            <Text className="p-3 text-white">{movieDetails?.overview}</Text>
            
          </View>

          <View>
            { movieCredits.cast && <CastSlider cast={movieCredits.cast} navigation={props.navigation} />}
          </View>
